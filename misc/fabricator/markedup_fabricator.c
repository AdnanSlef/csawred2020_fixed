...
#define LABELBUFSIZE 64  //unused, but is the length of id_prefix.
...
int validHashes(const char* buf1, const char* buf2, int bufLength){
    ...
    if (strncmp(md5hash1, md5hash2, MD5_DIGEST_LENGTH)){   //why strncmp??
        puts("MD5 hashes are not the same and should be!");//watch for nulls
        return 0;
    }
...
void runGame(){
    char idCard[IDCARDSIZE]; //just used for buffer overflow
    ...
    puts("Scanning...");
    if (strncmp(id1, id2, 0x40)!=0 || strncmp(id1, id_prefix,0x40)!=0){//more strncmp, maybe ok
    ...
    memcpy(idCard, id1, IDCARDBUFSIZE); //buffer overflow
    return;
}
