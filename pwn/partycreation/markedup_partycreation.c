...
typedef struct _character{      //24 bytes = sizeof(character)
    char name[NAMELENGTH]; //16
    char strength;         //17
    char dexterity;        //18
    char constitution;     //19
...
int getIntClean(){
    char input[MAXINTLENGTH];  //9-digit base10 numbers (or 8 digit negative)
...
void init(){
    ...
    int index = 0;
    while (index < 6){//initializes all 6 party members
        strcpy(party[index].name, "");//sets first name byte to null
...
void viewCharacter(){
    ...
    fflush(stdout);
    int index = getIntClean();//allows negative index (leak)
...
void renameCharacter(){
    ...
    fflush(stdout);
    int index = getIntClean();//allows negative index (write)
    ...
    if (index < 6){
        ...
        printf("Enter your character's name:\n>");
        fflush(stdout);
        getInput(NAMELENGTH, party[index].name);//accepts 15 bytes and null terminates
        puts("");
