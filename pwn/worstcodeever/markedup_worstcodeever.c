...
union identifier //I think it's 8 bytes
{
    char *name; //just a pointer
    int64_t id;
};

typedef struct Friend //may have padding causing it to be 16 bytes not 12
{
    union identifier identity; //a name pointer or an id
    int age;
} Friend;
...
void add_friend()
{
    ...
    if (human_name != 0)
    {
        ...
        fgets(name, NAME_LENGTH, stdin);//up to 63 bytes
        name[NAME_LENGTH] = 0;//null-terminate name string
        name[strcspn(name, "\n")] = 0;//null-terminate name at first newline

        f->identity.name = name;
    } else
    {
        puts("What is your friends barcode tag?");
        scanf("%ld", &f->identity.id);//hmm, could this be used as a pointer
    }
    ...
    friend_type[f_index] = human_name; //0 if id used; controlled by me if name used
...
void remove_friend()
{
    ...
    if (friend_type[index] != 0)//don't call free on robot id
        free(friend_list[index]->identity.name);
    free(friend_list[index]);

    puts("Your friend was removed"); //didn't affect f_index so probably UAF
                                     //Also could double free
}
